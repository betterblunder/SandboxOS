...
Available options:
    ...
    [-f file] - sets the wireless network name and key from a json file (with keys 'ssid' and 'password')
...
while getopts "d:f:i:m:n:s" o; do
    case "$o" in
        d)
            SDCARD_DEV=$OPTARG
            ;;
        ...
        m)
            ...
            ;;
        f)
            SSID=$(jq -r .ssid "$OPTARG")
            PSK=$(jq -r .password "$OPTARG")
            ;;
        n)
            IFS=":" 
            NETWORK=(${OPTARG})
